...
func main() {
	...
	go chef3(&wg, ch23, chEnd, 5) //Notice: chef3 is also responsible for telling other cooks when to stop.
...
func chef2(wg *sync.WaitGroup, ch12 <-chan int, ch23 chan<- int, chEnd <-chan bool, limit int) {
	...
	for {
		...
		/* The only condition for this chef to stop cooking and start waiting
		   for available cake is when there are none left. */
		if available2 == 0 {
			...
			// CHECKING GARNISHING CONDITION:
			/* Note: here the number of available cake is different from 0.*/
			if !cooking2 { // Checks if guarnishing cakes conditions are present.
				cooking2 = true
...
func chef3(wg *sync.WaitGroup, ch23 <-chan int, chEnd chan<- bool, limit int) {
	...
	for {
		...
		/* The only condition for this chef to stop cooking and start waiting
		   for available cake is when there are none left. */
		if available3 == 0 { // Checks to see if cakes are necessaire.
			...
			/* Note: here the number of available cake is different from 0.*/
			if !cooking3 { // Checks if decorating cakes conditions are present.
				cooking3 = true
